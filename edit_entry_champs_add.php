...
foreach ($overload_fields as $fieldname=>$fieldtype)
{
	...
	if ($overload_fields[$fieldname]["type"] == "textarea" )
		echo "<tr><td><textarea cols=\"80\" rows=\"2\" name=\"addon_".$overload_fields[$fieldname]["id"]."\">".htmlentities(removeMailUnicode($data),0,"UTF-8")."</textarea></td></tr>\n";
	else if ($overload_fields[$fieldname]["type"] == "text" )
		echo "<tr><td><input size=\"80\" type=\"text\" name=\"addon_".$overload_fields[$fieldname]["id"]."\" value=\"".htmlentities(removeMailUnicode($data),0,"UTF-8")."\" /></td></tr>\n";
	else if ($overload_fields[$fieldname]["type"] == "numeric" )
		echo "<tr><td><input size=\"20\" type=\"text\" name=\"addon_".$overload_fields[$fieldname]["id"]."\" value=\"".htmlentities(removeMailUnicode($data),0,"UTF-8")."\" /></td></tr>\n";
	else
	{
		...
		{
			echo "<option ";
			if (htmlentities(removeMailUnicode($data),0,"UTF-8") == trim($value,"&") || ($data == "" && $value[0]=="&"))
